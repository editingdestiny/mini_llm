...
class BPETokenizer:
    ...
    def train_from_text(
        ...
        for _ in range(max_merges):
            ...
            if best_freq < config.min_merge_freq:
                # Stop when no pairs meet the frequency threshold
                break
    ...
    def bpe_word_steps(self, word: str) -> List[dict]:
        """
        Return the step-by-step BPE merge history for a single word.

        Returns a list of step dicts, each with:
            step: int (1-based)
            pair: Tuple[str, str] — the pair being merged
            rank: int — merge rank (when this pair was learned)
            before: List[str] — symbols before this merge
            after: List[str] — symbols after this merge
            merged: str — the new merged token
        """
        symbols = list(word) + ["</w>"]
        if not self.merges:
            return []
    ...
    def encode_with_steps(self, text: str) -> dict:
        """
        Full encoding of text, with per-word BPE merge steps.
        ...
        Returns:
            {
              "words": [
                {
                  "word": "hello",
                  "steps": [...step dicts from bpe_word_steps...],
                  "final_tokens": List[str],
                  "token_ids": List[int],
                },
                ...
              ],
              "full_token_ids": List[int]  — including special tokens
            }
        """
        words = text.strip().split()
        result = {"words": [], "full_token_ids": [self.bos_id]}

        for w in words:
            word_steps = self.bpe_word_steps(w)
            word_tokens = self._bpe_word(w)

            # Fallback to char-level if any token is unknown
            if any(t not in self.token_to_id for t in word_tokens):
                word_tokens = list(w) + ["</w>"]

            token_ids = [self.token_to_id.get(t, self.unk_id) for t in word_tokens]
            ...
            result["full_token_ids"].extend(token_ids)

        result["full_token_ids"].append(self.eos_id)
        return result
